...
public class AttributeValidation {
  ...
  public static Mono<Void> validateName(String name) {
    if(name == null || name.isBlank()){
      return Mono.error(new BadRequestException(ATTRIBUTE_NAME_REQUIRED));
    }
    if(name.length() > ATTRIBUTE_NAME_LENGTH){
      return Mono.error(new BadRequestException(ATTRIBUTE_NAME_LENGTH_ERROR));
    }
    return Mono.empty();
  }
  ...
  public static Mono<Void> validateDescription(String description) {
    if(description == null || description.isBlank()){
      return Mono.error(new BadRequestException(ATTRIBUTE_DESCRIPTION_REQUIRED));
    }
    if(description.length() > ATTRIBUTE_DESCRIPTION_LENGTH){
      return Mono.error(new BadRequestException(ATTRIBUTE_DESCRIPTION_LENGTH_ERROR));
    }
    return Mono.empty();
  }
}
